...
API 1.1 adds everything needed for text-based in-editor games, shipped as addons. The shipped **Snake** sample (`Samples~/Addons/SnakeGame/SnakeGame.cs`, installed by Tools → Addons → Install Sample Addons) uses every member below and is the reference implementation.
