...
- **Rendered Markdown: relative file links work.** `[text](CHANGELOG.md)`-style links resolve against the document's folder and Ctrl+Click opens the file as an ATE tab (the context menu offers "Open in Text Editor" for them); link syntax whose target is neither a URL nor an existing file (anchors, dead paths) renders as plain text instead of a dead link.
- **Rendered Markdown: right-clicking a link offers "Open Link in Browser"** as the first item (with Copy Link URL beside it), in both locked and unlocked modes — the same open path as Ctrl+Click, ate:// links included.
