...
### Fixed
- **Rendered Markdown tabs now carry the full document context menu.** The rendered view's right-click menu only offered its clipboard/lock items (and unlocked rendered mode had no menu at all), so file-backed .md tabs were missing the Git submenu, Tabs navigation, Save As, Close Tab, and Show in File Explorer — all now appended in both locked and unlocked rendered modes (clicks inside an open block editor keep native handling; virtual rendered docs like release notes stay file/git-free). Source-mode tabs of any type (.md, .txt, …) already had the full menu.
