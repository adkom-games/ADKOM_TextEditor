...
- **Tools → Git** opens the Git window: working-tree changes with per-file staging checkboxes, a commit-message box, **Commit** and **Push** buttons, commit inspection (click a commit in the graph to see its files and message; on HEAD, edit the message and press **Amend** to rewrite it in place), and a **branch history graph** (vertical or horizontal, toggleable; each branch drawn in its own color; the horizontal layout stays vertically centered in the view) with hash/date/author/subject tooltips. The window survives domain reloads — the file list, graph, and your typed commit-message draft all come back (the draft also survives a commit-inspection round-trip). **Double-click a file** in the Changes list to diff it against its previous version: the working-tree view diffs against HEAD, and an inspected commit diffs against its parent. The Changes header carries a **Select ▾ menu** (All / None / Modified / Untracked / Added / Deleted / Renamed / Invert — sets the staging checkboxes of the filtered rows in one step) and a right-aligned **Filter** field that narrows the list to paths containing the text; each row's status letter is tinted like the gutter marks (green added/untracked, amber modified, red deleted, blue renamed), and **right-clicking a row** offers diff, open-in-editor, reveal, and the Git submenu — which for working-tree rows leads with the file's state-eligible operations (Stage / Unstage / Discard Changes…, the last confirmed before running) followed by Blame / File History / Time Lapse for that file. Inspecting a commit also fills read-only date / hash / author / title fields above the message box, Time Lapse-style.
