...
## 2026-08-02 — 0.14.0: the diff & merge release

- **Diff / Merge tool** (Editor/Diff/): DiffEngine (Myers O(ND) line diff with linear-space middle snake + degenerate-split guard, prefix/suffix intra-line spans, diff3-style Merge3 with end-inclusive insertion mapping and same-point-insertion conflicts); AteDiffWindow (Files/Folders/Tabs setup, framed splitter columns with center merge-button gutter, header padded to live scrollbar width via contentViewport GeometryChanged — the ListView itself never resizes when the scrollbar appears; folder scan capped at 20k/side, surfaced), three-way merge with serialized choices/result; AteDiffToolBridge (Library/ATE shim + request-file polling; InternalEditorUtility.SetCustomDiffToolPrefs via reflection, kDiffsDefaultApp="Custom Tool"). Every window reload-restores, merge-in-progress included.
- Git window double-click = diff vs previous version (working vs HEAD; inspected commit vs parent). Two-way merge buttons make git diffs selective reverts (Save Left writes the working file).
...
## 2026-08-06 — 1.1.0: the Time Lapse release

- **Git Time Lapse** (Editor/GitTimeLapseWindow.cs, multi-instance): a slider replays a file's ENTIRE git history — oldest commit to live tab buffer — in a read-only CodeView. Per-step diff highlights (DiffEngine vs the older neighbor; AddedBg/ChangedBg tints + git gutter bars, red stub for deletions), center-line held across content swaps (new CodeView.CenterLine, inverse of CenterOnLine), commit date/hash/author/title/full-message in read-only fields (GitService.CommitMessage = git show -s --format=%B), version x/y readout between ◀ ▶ revision steppers, ▲ ▼ change-region hopping with a Wrap Searches toggle, bottom status line with tab-vs-revision +/− counts, Copy to Tab via ApiReplaceRange (undoable while active).
- **Sliding-window fetching**: revision contents prefetch nearest-first only while the slider rests (~400 ms pause timer); contents outside the window are evicted (memory bounded); landing beyond the window fetches on demand. Generation counter outdates stale fetch loops; posted results are dropped if their slot slid out. Window size: per-window IntegerField override seeded from the new Time Lapse Window Size Options default (EditorConfig, 5–500, default 50).
- **Context menus everywhere, truly context-sensitive**: document menu hides inapplicable items (was: disabled); virtual tabs get a read-oriented menu (semantic navigation kept, edit/file/git dropped); Settings tab gets Tabs + Close Tab; AuxTextMenu attaches aux menus to the Time Lapse view (Time Lapse pruned — no recursion) and History preview (follows its selected doc); Diff rows get copy-left/right + per-side Git submenus. Parameterized git commands (GitBlameFor / GitFileHistoryFor with dropdown anchor / GitTimeLapseFor) let aux views act on THEIR file. AuxGotoPrompt: minimal goto-line for read-only views.
- Pragma audit on Cary's rule: shipped ATE/sample code must NEVER contain unknown pragmas (CS1633 floods consumer projects) — verified zero #pragma directives of any kind in package, Samples~, Addons~, and the upload mirror.
- 40 net new localized strings across the cycle (5 catalogs, 816 each); Manual §2/§11 updated; gates green (guards, l10n, signatures).

## 2026-08-06 — 1.1.1: the polish patch (same-day follow-up to 1.1.0)

- **Git window Changes pane**: Select ▾ bulk-staging menu (filtered rows; chunked add/restore — one giant command overran the OS arg limit and its failure echo garbled the console, now capped at AteConsole too), right-aligned Filter, gutter-palette status letters, per-row context menus with state-eligible ops (Stage/Unstage/Discard…), TL-style read-only commit meta fields, merge-commit file lists via first-parent diff-tree fallback, selection-mirroring status line, separator rules.
- **Branch graph**: lane 0 seeded to main (mid-work the newest ref claimed the trunk), segment-keyed colors (lane reuse made an unmerged stale branch read as merged), clickable horizontal ref labels, vertical centering of the horizontal layout.
- **Context menus**: hide-not-disable everywhere; virtual tabs read-oriented; Settings tab menu; AuxTextMenu for TL/History/Diff views (recursion pruned); MenuSink adapter (GenericMenu + DropdownMenu single-source builders); rendered-MD menu rebuilt via ContextualMenuPopulateEvent (the native selectable-label "Copy" menu displays after and over anything a MouseUp handler shows — replace, don't race).
- **Rendered Markdown**: per-line selection geometry via LineTopOf (headings are rich-text <size> spans inside 12px labels — cursor y sits at each line's BOTTOM and lines differ in height; per-label line height clipped tints and shrank hit zones), triple-click paragraph select, badge image-in-link rendering, link tooltips (title/alt + destination), Open Link in Browser item, relative file links resolved against BaseDir opening as tabs, dead link syntax unstyled.
- **AteTooltip**: shared cursor-following bordered tooltips across all eight window roots — TooltipEvent intercepted at TrickleDown before the element fills it, so the native popup never renders; declaration sites unchanged.
- **Misc**: status-bar selection readout (range or "Multiple Selections" + ch/wd/ln), author fields as "Name <email>" everywhere (LogEntry/GraphNode/BlameLine gained Email), File History as a focused tab, Show in File Explorer on file-backed tab-strip menus, TL prev/next-change hop with Wrap Searches, commit identity standardized to cary.farrier@adkomgames.com (unpushed 1.1.0-cycle commits re-authored via commit-tree).

## 2026-08-12 — 1.1.2: external-editor selection survives Unity version upgrades

- **Root cause** (reported from ALS on 6000.7.0a4): ATE registers as an IExternalCodeEditor under `EditorApplication.applicationPath`, and Unity persists that ABSOLUTE path in the External Script Editor pref (`kScriptsDefaultApp`). After the project moves to a new Unity version the pref still names the OLD install's Unity.exe; `TryGetInstallationForPath` compared with exact string equality, so ATE no longer claimed it, `CurrentCodeEditor` fell back to `UnityEditor.DefaultExternalCodeEditor` — which "opens" a script by LAUNCHING the configured app. A bare stale Unity.exe boots a second editor, grabs the last-used project (this repo), and dies on the "Multiple Unity instances cannot open the same project" lock dialog. Not a 6.7a4 bug; any version change triggers it.
- **Diagnosed live** against the running ALS editor over its MCP HTTP endpoint (script-execute reflection dump): pref=6000.3.19f1 path, app=6000.7.0a4 path, ATE registered but `claims current path: False`, current editor = DefaultExternalCodeEditor.
- **Fix**: `IsAteEditorPath()` claims ANY Unity Editor binary (`Unity.exe`/`Unity.app`/`Unity`, slash- and case-normalized) — a Unity binary set as the script editor can't mean anything else; a `delayCall` self-heal rewrites a stale selection to the running binary (deferred: other editors' InitializeOnLoad registrations may not have run yet); both fallback-editor pickers exclude Unity binaries by the same rule. Verified in the dev editor via MCP (matcher truth table) and healed ALS live (`SetExternalScriptEditor` + end-to-end `AssetDatabase.OpenAsset` → opened in ATE, no second instance).
