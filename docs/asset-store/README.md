...
- [x] **Compile on the newest supported Editor and the declared minimum with zero warnings.** Done. Verified: the declared minimum 6000.0.80f1 as a clean install with Semantic Features exercised (0.14.3); 6000.5.6f1 with zero `warning CS` including the Semantics compile (Editor.log audit); a clean-install 6000.5.6f1 console confirmed clear on 0.14.4 after the off-panel MeasureTextSize fix; and a clean install on the LTS line, 6000.3.21f1, with no console warnings or errors (all 2026-08-03). Re-verify both ends whenever the supported-version range changes — 0.14.2 proved the minimum can break silently when development happens on a newer editor (`ITextSelection.GetCursorPositionFromStringIndex`, `DropdownMenuSizeMode`, both 6000.3+).
- [x] **Baseline build-bleed test.** Done on 6000.5.6f1 (2026-08-03): player build with ATE 0.14.3 installed and Semantic Features enabled. Editor.log build report audited — zero ADKOM/CodeAnalysis assemblies shipped, zero package paths in Used Assets (the `System.Reflection.Metadata`/`System.Collections.Immutable` entries in the Mono dependency list are Unity profile members, evidenced by 12 never-shipped assemblies in the same list and the total absence of `Microsoft.CodeAnalysis`). Repeat only when the bleed surface changes: Roslyn bundle updated or install path moved, asmdef added/restructured, non-code assets added, or the supported Unity version raised.
